...
import { DataCubeEntryPoint, DataCube, Dimension } from "@zazuko/query-rdf-data-cube";
...
const getEntry = () => {
  ...
  return new DataCubeEntryPoint(SPARQL_ENDPOINT, {
    languages: [...locales],
    fetcher: {
      fetchOptions: {
        agent: proxyAgent,
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const fetchTest = async () => {
    ...
    console.time("cube");
    const cube = await entry.dataCubeByIri(
      "http://environment.ld.admin.ch/foen/px/0703030000_124/dataset"
    );
  ...
  switch (method) {
    case "GET":
      try {
        ...
        res.status(200).json(result.map(d => d.toJSON()));
      } catch (e) {
        console.error(e);
