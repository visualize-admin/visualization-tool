...
The goal is to establish a visual coherence between Visualize and other websites within the [admin.ch](https://www.admin.ch) domain.
...
Since Visualize serves specific use cases and therefore has unique requirements, we reserve the right to be non-compliant towards the use of so called «Fixed» elements, replacing them with our own components if needed, as well as adapting certain «Flexible» brand elements where necessary.
