...
expected, [the cube checker](/_cube-checker) can help you: it provides a list of
check against a cube that you can use to troubleshoot.
...
In the future, more checks could be implemented, for example checks on
dimensions, do not hesitate to
[post an issue](https://github.com/visualize-admin/visualization-tool/issues) if
you have an idea of a check.

⚠️ For now, the cube checker only supports versioned iris, so make sure the IRI
you're using has a version at the end.
