...
iframe.onload = () => {
  ...
  if (iframeWindow) {
    iframeWindow.postMessage(configuratorState, "*");
  }
};
`}
/>
...
will change. You can find their latest versions in the
...
`&flag\_\_debug=true` added to the end of the URL to enable the debug mode,
which will show the chart config directly below the chart that is being edited.
It also enables the `Dump to console` button, which will log the chart config to
the browser console, for easier re-use.
