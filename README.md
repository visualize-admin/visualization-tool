...
```sh
pnpm dev
pnpm dev:ssl # If you are working with the login process
```
...
> the redirection, or use the `pnpm dev:ssl` command to use HTTPS for the
...
> to use `pnpm e2e:ui:ssl` or `pnpm e2e:dev:ssl` in order to run tests locally
...
folder. The GitHub Action is generated by running the `pnpm run github:codegen`
...
the `options` object inside a given script and running `pnpm k6:codegen` to make
